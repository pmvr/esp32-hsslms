...
HSS_Priv::HSS_Priv(const std::vector<LMS_ALGORITHM_TYPE> &lmstypecodes,
                   const LMOTS_ALGORITHM_TYPE &lmotsAlgorithmType) : lmstypecodes(lmstypecodes), lmotsAlgorithmType(lmotsAlgorithmType) {
    priv.emplace_back(new LMS_Priv(lmstypecodes[0], lmotsAlgorithmType));
    pub.emplace_back(priv[0]->gen_pub());
    for (auto i=1; i<lmstypecodes.size(); i++) {
        priv.emplace_back(new LMS_Priv(lmstypecodes[i], lmotsAlgorithmType));
...
HSS_Priv::HSS_Priv(const HSS_Priv &obj) {
...
std::string HSS_Priv::sign(const std::string &message) {
    ...
    for (auto i=d; i<L; i++) {
        delete priv[i];
        priv[i] = new LMS_Priv(lmstypecodes[i], lmotsAlgorithmType);
